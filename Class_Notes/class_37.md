...
- Spring creates **only one instance** of a bean per Spring IoC container.
...
```java
context.getBean(HelloWorldMessage.class)
```

multiple times, you'll get the **same object**.

---

### **Eager Initialization (Default)**

* Beans are **created when the application context starts** (unless marked with `@Lazy`).
* Resources are allocated at startup.

---

## 2. **Changing the Default Behaviour**

### **Prototype Scope**

* Use `@Scope("prototype")` to create **a new instance every time** the bean is requested.
* Useful for non-shared objects.

### **Lazy Initialization**

* Use `@Lazy` to create the bean **only when first requested** instead of at context startup.

---

## 3. **Comparison Table**

| Feature            | Default Behaviour        | With Changes                  |
| ------------------ | ------------------------ | ----------------------------- |
| **Scope**          | Singleton                | Prototype / Request / Session |
| **Initialization** | Eager                    | Lazy                          |
| **Bean Sharing**   | Same instance everywhere | New instance for each request |
...
- Returns a **new instance** every time the bean is requested.
...
- Equivalent to traditional `applicationContext.xml`.
...
## 10. **@Lazy**
- Postpones bean creation until it is requested.
...
## 11. **@Primary**
- When multiple beans match a dependency, marks one as the default choice for autowiring.
...
- Translates database exceptions into Spring's `DataAccessException`.
