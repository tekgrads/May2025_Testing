...
3. **Validation**: The `validateAndSubmit` function checks each field. If empty, it sets the corresponding error message. It also focuses the first field with an error.

4. **Dynamic Rendering**: Radio buttons, dropdown options, and checkboxes are generated using `.map()` from reference arrays for easy scalability.

5. **Output**: If a user clicks Submit without filling a field, the respective error appears in red, and the focus moves to the first missing field. The form does not submit until all fields are correctly filled. Selected skills update dynamically as checkboxes are checked or unchecked.
