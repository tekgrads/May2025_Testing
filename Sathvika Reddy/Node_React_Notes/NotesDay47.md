...
* `useState([])` initializes an empty array `persons`.
* `useEffect` ensures the fetch call runs once when the component loads.
* `fetch('Person.json')` loads the JSON file, parses it with `.json()`, and stores it in state.
* Finally, `map` loops through `persons` and displays each object’s values.
...
* `a1` is a fixed array, while `a2` is managed with state.
* `.join(',')` converts array into a string separated by commas.
* React displays both arrays on the screen.
...
* `map` loops through the array and creates a `<span>` for each value.
* Each element needs a `key` to avoid React warnings.
* First case uses item as key, second uses index.
...
* First line displays all values.
* Second line filters numbers greater than 20.
* Third line filters numbers divisible by 10.
...
* `onChange` updates the respective state whenever user types.
* On clicking `Submit`, all data is combined into an object and logged.
...
* The state `person1` stores name, age, and gender.
* `Object.entries()` displays key-value pairs.
* On clicking Submit, `age` property is deleted and UI updates instantly.
...
* Two states are used: `email` and `password`.
* Inputs are controlled components; typing updates state.
* On Submit, both values are collected into an object and printed.
...
* Here, it sums all elements in `arr`.
* Initial value is `0`, so final sum is `150`.
...
* `sort((a, b) => a - b)` ensures numeric ascending order.
* Spread operator `[...]` avoids mutating the original array.
* `join(", ")` makes a clean string output.
...
* String arrays can be sorted alphabetically with default `sort()`.
* Spread operator ensures immutability.
* Output is a neat alphabetical list.
...
* `Object.keys()` extracts only property names.
* `join(", ")` makes them readable.
* The values are not shown, only keys.
...
* `Object.values()` extracts only property values.
* Very useful for data iteration.
* Keys are skipped, only values are shown.
...
* Using `map()`, each entry is displayed in `key : value` format.
* This is a clean way to render objects dynamically.
